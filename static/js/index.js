...
function line1() {  
    var myChart = echarts.init(document.querySelector(".line1")); // 确保HTML中有一个类名为.bar1的元素  
  
    // 假设yzdatabox和datedata1已经被正确定义和初始化  
    var data = yzdatabox;  
    // var datedata1 = ...; // 这里需要您提供或定义datedata1的内容  
  
    console.log(data, 'data');  
    console.log('Bar Chart Initialized?');  
  
    // 这个函数看起来是用于响应式设计的，但在此示例中未使用，您可以根据需要保留或删除  
    function fontSize(res) {  
    ...
    var option = {  
        ...
        xAxis: {  
            ...
            data: datedata1 // 确保这里的数据是类别名（如x轴标签）  
        },  
        yAxis: {  
            type: 'value',  
            axisLine: {  
        ...
        series: [{  
            data: data, // 确保这里的数据是对应的数值  
            type: 'bar', // 修改为'bar'来创建柱状图  
            barMaxWidth: '30%', // 可选，设置柱子的最大宽度  
            itemStyle: {  
                color: "#4B7EFE" // 设置柱子的颜色  
            }  
        }]  
    };  
...
function line4() {  
    var myChart = echarts.init(document.querySelector(".line4")); // 确保HTML中有一个class为bar4的元素  
  
    data = gmdatabox; // 确保gmdatabox已定义并包含正确的数据  
  
    function fontSize(res) {  
        let clientWidth = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;  
        if (!clientWidth) return;  
        let fontSize = 100 * (clientWidth / 1920);  
        return res * fontSize; // 这个函数目前未在代码中使用，可以根据需要调整  
    };  
    ...
    option4 = {  
        ...
        xAxis: {  
            ...
            data: datedata4 // 确保datedata4已定义并包含正确的类别数据  
        },  
        ...
        series: [{  
            data: data, // 确保data已定义并包含正确的数值数据  
            type: 'bar', // 修改为'bar'以绘制柱状图  
            barMaxWidth: '20px', // 可选，设置柱子的最大宽度  
            barGap: '30%', // 可选，设置柱子之间的间隔百分比  
            itemStyle: {  
                color: "#4B7EFE" // 设置柱子的颜色  
            }  
        }]  
